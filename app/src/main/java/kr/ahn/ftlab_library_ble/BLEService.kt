...
class BLEService: Service() {
    ...
    fun connect(address: String?): Boolean {
        try {
            ...
            val device = mBluetoothAdapter!!.getRemoteDevice(address) ?: //Check whether a device was returned
    ...
    private fun broadcastUpdate(action: String) {
        Log.i(TAG, "broadcastUpdate222= $action")
    ...
    private fun broadcastUpdate(action: String, characteristic: BluetoothGattCharacteristic) {
        ...
        val intent = Intent(BLEConstants.ACTION_DATA_PARSE)
        var dataValueHex = ""
        var dataValueNew = ""

        val recValue = characteristic.value
        val convertData = IntArray(recValue.size)
        if (recValue != null) {
            for ((index, value) in recValue.withIndex()) {
                convertData[index] = value.toUByte().toInt()
            }

            for (singleData in convertData) {
            ...
            Log.i(TAG, "MESSAGE Response Array Normal===> size: " + recValue.size +",  " + dataValueNew + " UUID " + characteristic.uuid)
            intent.putExtra(BLEConstants.EXTRA_DATA, recValue)
